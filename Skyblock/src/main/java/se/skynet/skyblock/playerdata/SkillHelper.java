...
public class SkillHelper {
    ...
    public static double calculateDamageMultiplyerFromCombatLevel(int combatLevel){
        int[] healthBonusTeirs = new int[]{50 , 10};
        double[] healthBonus = new double[]{4, 1};

        return calculateSkillBonus(combatLevel, healthBonusTeirs, healthBonus) / 100;
    }
    ...
    public static double calculateStrengthBonusFromCombatLevel(int level) {
        // level 1-14 gives 2, 15-19 gives 3, 20-25 gives 4, 26-60 gives 5
        int[] strengthBonusTeirs = new int[]{14, 46};
        double[] strengthBonus = new double[]{1, 2};

        return calculateSkillBonus(level, strengthBonusTeirs, strengthBonus);
    }
}
